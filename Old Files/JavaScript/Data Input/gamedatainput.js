...
window.onload = function loadPlayerStatus() {
	//document.getElementById("inputpagetitle").innerHTML = maindata["Input Date"][0] + ": " + maindata["Input Date"][1];
	var roster = document.getElementById("roster");

	var playingul = document.getElementById("playingul");
	var subul = document.getElementById("subul");
	var injuredul = document.getElementById("injuredul");
	var absentul = document.getElementById("absentul");

	for (var i = 0; i < maindata["Player"].length; i++) {
		var playername = maindata["Player"][i]["Name"].toLowerCase().replace(" ","");

		if (maindata["Games"]["Player Status"][0][maindata["Player"][i]["Name"]] === "Playing" || maindata["Games"]["Player Status"][0][maindata["Player"][i]["Name"]] === "Sub") {
		...
		li.setAttribute("id", '' + playername + 'li');
		li.appendChild(document.createTextNode('' + maindata["Player"][i]["Name"] + ''));

		switch (maindata["Games"]["Player Status"][maindata["Games"]["Player Status"].length - 1][maindata["Player"][i]["Name"]]) {
			case "Playing":
				playingul.appendChild(li);
	...
	newbutton.setAttribute('id','changestatusbutton');
	newbutton.setAttribute('onclick','changePlayerStatus()');
...
function subPlayers() {
	...
	for (var i = 0; i < maindata["Player"].length; i++) {
		...
		if (maindata["Games"]["Player Status"][maindata["Games"]["Player Status"].length - 1][maindata["Player"][i]["Name"]] === "Playing" || maindata["Games"]["Player Status"][maindata["Games"]["Player Status"].length - 1][maindata["Player"][i]["Name"]] === "Sub") {
			document.getElementById('' + currentplayer + 'radiodiv').style.display = "block";
		}
	}
...
function changePlayerStatus() {
	...
	for (var i = 0; i < maindata["Player"].length; i++) {
		var currentplayer = maindata["Player"][i]["Name"].toLowerCase().replace(" ","");
		var playerelmnt = document.getElementById('' + currentplayer + 'li');

		if (maindata["Games"]["Player Status"][maindata["Games"]["Player Status"].length - 1][maindata["Player"][i]["Name"]] === "Playing" || maindata["Games"]["Player Status"][maindata["Games"]["Player Status"].length - 1][maindata["Player"][i]["Name"]] === "Sub") {

			if (document.getElementById('' + currentplayer + 'playingradio').checked) {
				newplayerstatus[maindata["Player"][i]["Name"]] = "Playing";
				playerelmnt.parentElement.removeChild(playerelmnt);
				document.getElementById('playingul').appendChild(playerelmnt);
			}

			else if (document.getElementById('' + currentplayer + 'subradio').checked) {
				newplayerstatus[maindata["Player"][i]["Name"]] = "Sub";
				playerelmnt.parentElement.removeChild(playerelmnt);
				document.getElementById('subul').appendChild(playerelmnt);
			}

			else if (document.getElementById('' + currentplayer + 'injuredradio').checked) {
				newplayerstatus[maindata["Player"][i]["Name"]] = "Injured";
				playerelmnt.parentElement.removeChild(playerelmnt);
				document.getElementById('injuredul').appendChild(playerelmnt);
			}
	...
	var dt = new Date();
	newplayerstatus["Date-Time"] = [dt.getUTCHours(), dt.getUTCMinutes(), dt.getUTCSeconds(), dt.getUTCMilliseconds()];

	maindata["Games"]["Player Status"].push(newplayerstatus);

	console.log(maindata);

	fs.writeFile("Data Storage/maindata.JSON", JSON.stringify(maindata, null, "\t"), function (err) {
        if (err != undefined) {
            alert(err.message,"Lineup Change Save error")
        }
    });
...
function inputData() {
	...
	if (pressure === "Drill" && shotwasattempted === 0) {
		...
		if (passoccurred === 0 && dribbleoccurred === 0 && (abletopass + abletoshoot + abletodribble + abletorebound) === 0 && passwasattempted === 0) {
			...
			maindata["Practices"][indexofpractice]["Data"][practicingarray.indexOf(currentplayer)]["GamePressure"].push(newevent);	
			...
			maindata["Practices"][indexofpractice]["Data"][practicingarray.indexOf(currentplayer)]["GamePressure"].push(newevent);
			maindata["Practices"][indexofpractice]["Data"][practicingarray.indexOf(player1)]["GamePressure"].push(newevent2);
	...
	if (pressure === "Game") {
		if (currentevent !== 0) {
			for (var i = 0; i < practicingarray.length; i++) {
				var arraylength = maindata["Practices"][indexofpractice]["Data"][i]["GamePressure"].length
				for (var j = 0; j < arraylength; j++) {
					if (maindata["Practices"][indexofpractice]["Data"][i]["GamePressure"][maindata["Practices"][indexofpractice]["Data"][i]["GamePressure"].length - 1]["Sequence"] === currentsequence) {
						maindata["Practices"][indexofpractice]["Data"][i]["GamePressure"].pop();
					}
				}
			}
			...
			for (var i = 0; i < practicingarray.length; i++) {
				var arraylength = maindata["Practices"][indexofpractice]["Data"][i]["GamePressure"].length
				for (var j = 0; j < arraylength; j++) {
					if (maindata["Practices"][indexofpractice]["Data"][i]["GamePressure"][maindata["Practices"][indexofpractice]["Data"][i]["GamePressure"].length - 1]["Sequence"] === currentsequence - 1) {
						maindata["Practices"][indexofpractice]["Data"][i]["GamePressure"].pop();
					}
				}
			}
